...
export default class Bus extends Instance
{
	 ...
	 *   ->get_output_stream():Stream - get stream to listen the bus.
	 * 
	 *   ->post(arg_msg:DistributedMessage):boolean - send a DistributedMessage instance.
	 *   ->subscribe(arg_filter:string|object, arg_handler:function):nothing - subscribe to messages of the bus.
	 * 
	 *   ->has_locale_target(arg_target:string):boolean - has target recipient?
	 *   ->add_locale_target(arg_target:string):nothing - add a target recipient.
	 * 
	 *   ->add_gateway(arg_gateway:BusGateway):nothing - add a bus gateway.
	 *   ->get_gateways():array of BusGateway - get bus gateways.
	 * 
	 *   ->dispatch_on_gateways(arg_msg:DistributedMessage):boolean - dispach given message to bus gateways.
	 * 
